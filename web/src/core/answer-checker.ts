...
function align(
  ...
  const recognized = a.map((letter, i) => {
    ...
    for (let j = 0; j <= b.length; j++) {
      ...
      if (
        j < b.length &&
        costs[i][j] + Number(letter !== b[j]) + remaining[i + 1][j + 1] ===
          costs[a.length][b.length]
      )
        observations.add(letter === b[j]);
    }
  ...
  while (i || j) {
    ...
    if (
      ...
      alignment.push({
        ...
        actual: b[j - 1],
        expectedIndex: i - 1,
        operation: a[i - 1] === b[j - 1] ? 'match' : 'replace',
      });
      i--;
      j--;
  ...
  return {
    distance: costs[a.length][b.length],
...
export function evaluate(
  ...
  const isCyrillic = cyrillicPattern.test(normalizedAnswer);
  const mixed = isCyrillic && latinPattern.test(normalizedAnswer);
  ...
  const units = (word.units ?? []).map((u, position) => {
    ...
    const parts = best.alignment.filter(
      (a) =>
        a.expectedIndex !== null &&
        a.expectedIndex >= start &&
        a.expectedIndex < offset,
    );
    ...
    return {
      ...
      position,
      expected,
      actual: parts.map((p) => p.actual).join(''),
      observation: unknown
        ? 0
        : mixed || canonical !== best.expected
          ? null
          : evidence.includes(false)
            ? 0
            : evidence.includes(null)
              ? null
              : 1,
    };
  });
  const ambiguous =
    mixed ||
    (!correct &&
      (units.some((u) => u.observation === null) ||
        candidates.some((c) => c !== best && c.distance === best.distance)));
  return {
    status: unknown
      ? 'unknown'
      : correct
        ...
        : ambiguous
          ? 'ambiguous'
          : best.distance < best.expected.length / 2
            ? 'partial'
            : 'incorrect',
    correct,
