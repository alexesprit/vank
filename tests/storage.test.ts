...
it('creates only MVP stores and persists settings and full attempt events across reopening', async () => {
  ...
  const result = completeAttempt(state, { word, phase: 'bootstrap' }, 'такси', false, 'client-1', 10, 20, 'attempt-1');
  await repo.saveAttempt(result.attempt, result.state);
  repo.close(); repo = await openRepository(name);
  ...
  expect([...db.objectStoreNames]).toEqual(['attempts', 'letterStats', 'settings', 'wordStats']);
  db.close(); repo.close();
});
it('writes attempts and progress atomically, rejecting duplicate event IDs without double counting', async () => {
  ...
  const result = completeAttempt(await repo.loadState(), { word: deriveWord('ՄԱՄԱ'), phase: 'bootstrap' }, 'mama', false, 'c', 1, 2, 'same-id');
  await repo.saveAttempt(result.attempt, result.state);
  const duplicate = completeAttempt(result.state, { word: deriveWord('ՄԱՄԱ'), phase: 'bootstrap' }, '', true, 'c', 3, 4, 'same-id');
  await expect(repo.saveAttempt(duplicate.attempt, duplicate.state)).rejects.toThrow();
