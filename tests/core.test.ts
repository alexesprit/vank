...
import { selectWord, unknownLetters, personalDifficulty } from '../web/src/core/word-selector';
...
describe('answer evaluation', () => {
  ...
  it('attributes a final mistake only to its unit', () => {
    ...
    expect(simple.alignment.at(-1)).toMatchObject({ expected: 'и', actual: 'о', operation: 'replace' });
  });
  it('maps multichar sounds and digraphs, handles insertions/deletions', () => {
    expect(evaluate(word('ՇՈՒՆ'), 'shun').units.map(u => u.observation)).toEqual([1, 1, 1]);
    const insertion = evaluate(word('ՏԱՔՍԻ'), 'taaksi');
    expect(insertion.alignment.some(a => a.operation === 'insert')).toBe(true);
    expect(insertion.correct).toBe(false);
    expect(evaluate(word('ՏԱՔՍԻ'), 'tasi').units.find(u => u.source === 'Ք')?.observation).toBe(0);
  });
  it('does not invent letter evidence for unmapped accepted aliases', () => {
    ...
    expect(evaluate(pizza, 'pizza').units.every(u => u.observation === null)).toBe(true);
    const unmapped = word('ՏԱՔՍԻ'); delete unmapped.units;
...
describe('learning evidence', () => {
  it('applies EMA with familiarity discount and only punishes the misread letter', () => {
    const w = word('ՏԱՔՍԻ', 1), state = known([w], 0.5);
    ...
    expect(updateScores(state, low, evaluate(low, 'такси'), 100).letters.Տ.score).toBeCloseTo(0.575);
  });
  it('keeps repeated glyphs to one observation per attempt and protects mastery from guesses', () => {
    const w = word('ՄԱՄԱ', 1);
    let state = empty();
    for (let i = 0; i < 50; i++) state = updateScores(state, w, evaluate(w, 'mama'), i);
  ...
  it('records exposure but no credit on skip; digraph updates both written letters', () => {
    ...
    expect(skipped.letters.Շ).toMatchObject({ score: 0, attempts: 1, correct: 0 });
  });
});
...
describe('adaptive selection', () => {
  ...
  it('prefers weak letters over otherwise identical strong words', () => {
    ...
    expect(personalDifficulty(words[1], state)).toBeGreaterThan(personalDifficulty(words[0], state));
  });
  it('never introduces multiple unknown letters after bootstrap', () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      expect(unknownLetters(selected.word, state).length).toBeLessThanOrEqual(1);
      const result = completeAttempt(state, selected, selected.word.readingLatin, false, 'client', i, i + 1, `id-${i}`);
      Object.assign(state, result.state);
    }
  });
  it('avoids immediate repeats and reinforces an introduction with other words', () => {
    const words = [word('ՄԱՄԱ'), word('ՆԱՆԱ'), word('ՄԱՍ'), word('ՍԱ'), word('ՍԱՄ')];
    const state = known(words.slice(0, 2));
    const result = completeAttempt(state, { word: words[2], phase: 'introduction', introducedLetter: 'Ս' }, 'mas', false, 'c', 1, 2, 'a');
...
it('creates rich immutable attempt events and real progress metrics', () => {
  ...
  const result = completeAttempt(empty(), { word: w, phase: 'bootstrap' }, ' такси ', false, 'client-1', 100, 500, 'attempt-1');
  expect(result.attempt).toMatchObject({ id: 'attempt-1', clientId: 'client-1', type: 'attempt.completed', schemaVersion: 1, timestamp: 500,
    payload: { wordId: w.id, answer: ' такси ', expected: 'такси', correct: true, shownAt: 100, answeredAt: 500 } });
  expect(result.state.words[w.id]).toMatchObject({ attempts: 1, correct: 1, lastSeenAt: 500 });
  expect(progress(result.state)).toMatchObject({ introduced: 5, strong: 0, accuracy: 1, verifiedAccuracy: 1, skips: 0 });
});
