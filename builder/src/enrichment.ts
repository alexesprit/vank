...
export function enrichmentSchema(languages: string[], ids?: string[]) {
  ...
  const languageMap = (value: object) => ({
    type: 'object',
    properties: Object.fromEntries(languages.map((l) => [l, value])),
    required: languages,
    additionalProperties: false,
  });
  const properties = {
    id: { ...string, ...(ids ? { enum: ids } : {}) },
    meaning: languageMap(string),
    familiarity: languageMap(number),
    loanwordScore: number,
    usefulnessScore: number,
...
export function parseAiResponse(
  ...
  if (
    Object.keys(response).some((k) => k !== 'items') ||
    !Array.isArray(response.items)
  )
    throw new Error('Expected AI items array');
  const found = new Set<string>(),
    allowed = Object.keys(
      enrichmentSchema(languages).properties.items.items.properties,
    );
  const items = response.items.map((value) => {
    const item = object(value);
    if (
      Object.keys(item).some((k) => !allowed.includes(k)) ||
      allowed.some((k) => !(k in item))
    )
      throw new Error('Unexpected or missing AI fields');
    if (
      typeof item.id !== 'string' ||
      !ids.includes(item.id) ||
      found.has(item.id)
    )
      throw new Error('Unknown or duplicate AI ID');
    found.add(item.id);
    validateMetadata(item);
    for (const key of ['meaning', 'familiarity']) {
      const map = object(item[key]);
      if (
        languages.some((l) => !(l in map)) ||
        Object.keys(map).some((l) => !languages.includes(l))
      )
        throw new Error('Missing or unexpected learner language');
    }
...
export async function enrichWords(
  ...
  if (
    ...
    throw new Error(
      'Provider, model and unique learner languages are required',
    );
      ...
      for (let attempt = 0; ; attempt++) {
        try {
          ...
          if (typeof content !== 'string')
            throw new Error(
              `${providerLabel(provider)} returned no JSON content`,
            );
