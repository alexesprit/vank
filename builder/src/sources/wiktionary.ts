...
export function wiktionaryRecord(
  value: unknown,
  options: {
    priority?: number;
    datasetUrl?: string;
  ...
  if (
    ...
    throw new Error('Malformed Wiktionary record');
  if ([...record.word].length < 2) return null;
  if (
    record.pos === 'name' &&
    !options.allowedNames?.has(normalizeArmenian(record.word))
  )
    return null;
  if (
    [
  ...
  return {
    ...
    source: {
      type: 'wiktionary',
      name: `${edition} Wiktionary contributors${options.datasetUrl ? `; dump: ${options.datasetUrl}` : ''}`,
      url: `https://${edition}.wiktionary.org/wiki/${encodeURIComponent(record.word)}#Armenian`,
