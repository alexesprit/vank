...
const words = input.map((value, index) => {
  ...
  return {
    ...
    stableId: w.id,
    word: w.word,
    reading: w.readingLatin,
    definitions: strings(r.rawDefinitions).slice(0, 3),
  };
});
...
const batches = Array.from({ length: Math.ceil(words.length / 400) }, (_, i) =>
  words.slice(i * 400, (i + 1) * 400),
);
...
async function worker() {
  while (next < batches.length) {
    const index = next++,
      batch = batches[index],
      ids = batch.map((w) => w.id);
    const hash = createHash('sha256')
      .update(JSON.stringify({ model, prompt, language, batch }))
      .digest('hex');
    const path = `builder/cache/discovery/${hash}.json`;
    if (existsSync(path))
      results[index] = parseDiscovery(
      ...
      for (let attempt = 0; ; attempt++) {
        try {
            ...
            'https://openrouter.ai/api/v1/chat/completions',
            {
              ...
              body: JSON.stringify({
                ...
                response_format: {
                  type: 'json_schema',
                  json_schema: {
                    name: 'recognition_candidates',
                    strict: true,
                    schema: {
                      ...discoverySchema,
                      properties: {
                        items: {
                          ...discoverySchema.properties.items,
                          items: {
                            ...discoverySchema.properties.items.items,
                            properties: {
                              ...discoverySchema.properties.items.items
                                .properties,
          ...
          await writeJson(path, { items: results[index] });
          break;
        } catch (error) {
    ...
    console.log(
      `discovery ${++complete}/${batches.length}; batch ${index + 1}: ${results[index].length} proposals`,
    );
  }
}
...
await writeJson(
  'builder/data/discovery-proposals.json',
  results.flat().map((item) => {
    const word = byId.get(item.id);
    if (!word) throw new Error(`Unknown discovery ID: ${item.id}`);
    return { ...item, id: word.stableId, word: word.word };
  }),
);
